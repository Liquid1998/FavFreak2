...
# Example fingerprints dictionary — extend this or load from JSON
FINGERPRINTS = {
...
def print_fingerprints(results_map):
    ...
    for hash_val, urls in results_map.items():
        if hash_val in FINGERPRINTS:
            ...
            print(f"\u001b[31m[{tech}]\u001b[0m {hash_val} - count: {len(urls)}")
            for url in urls:
                print(f"     {url}")
...
def main():
    ...
    start_time = timer()
    results_map = analyze_favicons(urls, append_favicon)
